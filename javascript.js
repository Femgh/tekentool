...
function drawing(e) {
    if(!isDrawing) { // als isDrawing niet waar is wordt alles hieronder niet uitgevoerd
        return;
    } 
...
toolKnoppen.forEach(function(btn) {
    btn.addEventListener("click", function () {
        document.querySelectorAll(".tool").forEach(function(toolBtn) {
            toolBtn.classList.remove("active"); // verwijdert eerst de 'active' klasse van alle toolknoppen
        });
        btn.classList.add("active"); // dan weer toevoegen
        selectedTool = btn.id;
    });
});
